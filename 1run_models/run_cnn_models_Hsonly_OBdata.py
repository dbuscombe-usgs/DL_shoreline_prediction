...
for n_steps_in in [20,30,40,50,60,70,80]:
    ...
    for (index, colname) in enumerate(yresults):
        ...
        plot_model(
            model,
            to_file=f"CNN_f_microcnn_OceanBeach_p"+str(trainable_count)+'b'+str(batch)+'k'+str(kernel)+".png",
            show_shapes=True,
            show_dtype=False,
    ...
    # ymetrics= pd.DataFrame(index=np.arange(4),
    #                     columns=['rmse_arr','pear_arr','mielke_arr'])
    ...
    myFmt = mdates.DateFormatter('%Y')
    plt.gca().xaxis.set_major_formatter(myFmt)
    plt.text(dates[int(3.5*len(dates)/4)], -70, f'RMSE (test) = {best_rmse}-m')
    plt.xlabel('Time')
    plt.ylabel('Shoreline position (m)')
    ...
    # plt.show()
    plt.savefig(f'./output/CNN_f_microcnn_OceanBeach_nn{num_neurons}_p'+str(trainable_count)+'b'+str(batch)+'k'+str(kernel)+'_linearact_DT'+str(n_steps_in)+'_model_explore1.png', dpi=300, bbox_inches='tight')
    plt.close('all')
    ...
    # plt.show()
    plt.savefig(f'./output/CNN_f_microcnn_OceanBeach_nn{num_neurons}_p'+str(trainable_count)+'b'+str(batch)+'k'+str(kernel)+'_linearact_DT'+str(n_steps_in)+'_modelexplore2.png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plot_model(
        best_model,
        to_file=f"CNN_f_microcnn_OceanBeach_p"+str(trainable_count)+'b'+str(batch)+'k'+str(kernel)+'DT'+str(n_steps_in)+".png",
        show_shapes=True,
        show_dtype=False,
        show_layer_names=False,
        rankdir="TB",
