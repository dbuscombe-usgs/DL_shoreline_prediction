...
def cnn_custom(train_x, train_y, dev_x, dev_y, cfg): #, second_ccn_bank, linear_activation, num_neurons):
    ...
    model.add(Conv1D(filters=n_filters, kernel_size=n_kernels,
                    activation='relu', input_shape=(n_steps_in, n_features))) #relu
...
for n_steps_in in [20,30,40,50,60,70,80]:
    ...
    for (index, colname) in enumerate(yresults):
        ...
        model,history = cnn_custom(train_x, train_y, dev_x, dev_y, cfg_list[index])
        #, second_ccn_bank, linear_activation, num_neurons) 
        # print(model.summary())
    ...
    yresults.to_csv(f'./output/CNN_f_microcnn2_OceanBeach_nn{num_neurons}_p{trainable_count}_linearact_DT{n_steps_in}.csv')
    ...
    ymetrics.to_csv(f'./output/CNN_f_microcnn2_OceanBeach_nn{num_neurons}_p{trainable_count}_DT{n_steps_in}_linearact_metrics.csv')
    ...
    # plt.show()
    plt.savefig(f'./output/CNN_f_OceanBeach_microCNN2_ts_DT{n_steps_in}_params{num_params}.png', dpi=200, bbox_inches='tight')
    plt.close()
    ...
    # plt.show()
    plt.savefig(f'./output/CNN_f_microcnn2_OceanBeach_nn{num_neurons}_p'+str(trainable_count)+'b'+str(batch)+'k'+str(kernel)+'_linearact_DT'+str(n_steps_in)+'_model_explore1.png', dpi=200, bbox_inches='tight')
    plt.close('all')
    ...
    # plt.show()
    plt.savefig(f'./output/CNN_f_microcnn2_OceanBeach_nn{num_neurons}_p'+str(trainable_count)+'b'+str(batch)+'k'+str(kernel)+'_linearact_DT'+str(n_steps_in)+'_modelexplore2.png', dpi=200, bbox_inches='tight')
    plt.close()
