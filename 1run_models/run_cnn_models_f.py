...
## dy
# mat_out= pd.DataFrame(np.gradient(mat_in.yout.values), index=mat_in.index)
...
for (index, colname) in enumerate(yresults):
    ...
    plt.ylabel('Shoreline position (m)')
    # plt.show()
    plt.savefig(f'./output/CNN_f_microcnn__nn{num_neurons}_p'+str(trainable_count)+'b'+str(batch)+'k'+str(kernel)+'_linearact', dpi=200, bbox_inches='tight')
    plt.close()
...
yresults.to_csv(f'./output/CNN_f_microcnn_nn{num_neurons}_p{trainable_count}_linearact.csv')
...
ymetrics.to_csv(f'./output/CNN_f_microcnn__nn{num_neurons}_p{trainable_count}_linearact_metrics.csv')


best_model = MODELS[4] #2]
...
plt.text(65, 50, 'RMSE (test) = 5.6-m')
...
plt.ylabel('Shoreline position (m)')
# plt.show()
plt.savefig(f'./output/CNN_f_microCNN_ts.png', dpi=200, bbox_inches='tight')
plt.close()
