...
def cnn_custom(train_x, train_y, dev_x, dev_y, cfg, second_ccn_bank, linear_activation, num_neurons):
    ...
    # fit model
    es = EarlyStopping(patience=20, verbose=0, min_delta=min_delta,
...
yresults= pd.DataFrame(index=mat_in[ plot_date :mat_in.index[-1]].index,
                    columns=['ann1','ann2','ann3','ann4','ann5',
                                'ann6','ann7','ann8','ann9','ann10','ann11','ann12',
                                'ann13','ann14','ann15','ann16','ann17','ann18','ann19',
...
for (index, colname) in enumerate(yresults):
    ...
    plt.ylabel('Shoreline position (m)')
    # plt.show()
    plt.savefig(f'./output/CNN_f_shoreshop_microcnn_nn{num_neurons}_p'+str(trainable_count)+'b'+str(batch)+'k'+str(kernel)+'_linearact', dpi=200, bbox_inches='tight')
    plt.close()
...
yresults.to_csv(f'./output/CNN_f_shoreshop_microcnn_nn{num_neurons}_p{trainable_count}_linearact.csv')
...
ymetrics.to_csv(f'./output/CNN_f_shoreshop_microcnn__nn{num_neurons}_p{trainable_count}_linearact_metrics.csv')
...
plt.text(210, 170, 'RMSE (test) = 8.5-m')
...
plt.ylabel('Shoreline position (m)')
# plt.show()
plt.savefig(f'./output/CNN_f_shoreshop_microCNN_ts.png', dpi=200, bbox_inches='tight')
plt.close()
